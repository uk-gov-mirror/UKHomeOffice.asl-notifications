...
module.exports = settings => {
  ...
  const { Profile, Role, Project, Establishment } = db(settings.db);

  const getType = (req, model) => {
    const task = req.body.data;

    if (get(task, 'action') === 'revoke') {
  ...
  const getProfiles = (req, type) => {
    switch (type) {
      ...
      case 'ntco':
        return getNTCOS(req);
      case 'outgoingNtco':
        return getNTCOS(req, 'data.establishment.from.id');
    }
    return Promise.resolve();
  };
  ...
  const getApplicants = req => {
    return Promise.resolve()
      ...
      .then(profiles => profiles.filter(p => !p.asruUser))
      .then(profiles => {
        if (!profiles.length) {
          throw new NotFoundError();
        }
  ...
  const getNTCOS = (req, establishmentIdPath = 'data.establishmentId') => {
    return Promise.resolve()
      .then(() => {
        const establishmentId = get(req.data, establishmentIdPath);
        if (!establishmentId) {
          throw new NotFoundError();
        }
  ...
  const getRecipients = (req, res, next) => {
    Promise.all([
      ...
      .then(([ action, status, outgoing ]) => {
        req.recipients = { action, status, outgoing };
      })
      .then(() => next())
      .catch(next);
  };
  ...
  const notify = (req, res, next) => {
    const notifications = flatten([
      ...
      ...req.recipients.outgoing.map(profile => ({
        profile,
        subject: 'Status change',
        template: 'notification-task-outgoing'
      }))
    ]);
