...
module.exports = ({ schema, logger, task }) => {
  ...
  const ignoredActions = ['create', 'delete'];
      ...
      return Promise.resolve()
        ...
        .then(() => {
          if (taskHelper.isGranted(task)) {
            // notify any other associated establishment admins that a PIL has been updated
            return getNonOwningAdmins(applicantId)
              .then(otherAdmins => {
                otherAdmins.map(profile => {
                  logger.verbose('new application/amendment granted, notifying non-owning-establishment admins');
                  notifications.set(profile.id, { emailTemplate: 'task-closed', applicant, recipient: profile });
                });
              });
          }
