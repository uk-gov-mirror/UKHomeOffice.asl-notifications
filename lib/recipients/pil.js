...
module.exports = async ({ schema, logger, task }) => {
  ...
  const notifyOutgoingNTCOs = params => {
    ...
    outgoingNtcos.forEach(ntco => notifications.set(ntco.id, { ...params, recipient: ntco }));
  };
  ...
  const notifyOutgoingAdmins = params => {
    ...
    outgoingAdmins.forEach(admin => notifications.set(admin.id, { ...params, recipient: admin }));
  };

  const notifyNonOwningAdmins = params => {
    logger.verbose(`${params.logMsg}, notifying non-owning establishment admins`);
    nonOwningAdmins.forEach(admin => notifications.set(admin.id, { ...params, recipient: admin, establishmentId: admin.commonEstablishment.id }));
  };
  ...
  if (taskHelper.isWithApplicant(task)) {
    ...
    notifyNTCOs(withApplicantParams);
    return notifications;
  }
