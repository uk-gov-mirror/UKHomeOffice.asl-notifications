...
module.exports = async ({ schema, logger, task, publicUrl }) => {
  ...
  if (task.data.action.match(/^rop-reminder-/)) {
    const when = get(task, 'data.when');
    const projectId = task.data.project.id;
  ...
  if (taskHelper.isSentToApplicant(task)) {
    const sentToApplicantParams = {
      ...params,
      emailTemplate: 'conditions-to-applicant',
      subject: 'New conditions have been submitted to your PPL',
      logMsg: 'project conditions sent to licence holder'
    };

    notifyUser(applicant, sentToApplicantParams);
    return notifications;
  }

  if (taskHelper.isAcceptedConditions(task)) {
    const conditionsAcceptedParams = {
      ...params,
      emailTemplate: 'conditions-accepted',
      subject: 'Conditions imposed on PPL have been accepted.',
      logMsg: 'licence holder accepted project conditions'
    };

    const inspector = await Profile.query().findById(task.data.changedBy);
    notifyUser(inspector, conditionsAcceptedParams);
    return notifications;
  }

  if (taskHelper.isRepresentationsMade(task)) {
    const representationsMadeParams = {
      ...params,
      emailTemplate: 'representations-made',
      subject: 'Licence holder has made representations for new conditions.',
      logMsg: 'licence holder made representations for project conditions'
    };

    const inspector = await Profile.query().findById(task.data.changedBy);
    notifyUser(inspector, representationsMadeParams);
    return notifications;
  }
