...
module.exports = async ({ schema, logger, task }) => {
  ...
  if (taskHelper.isExpired(task)) {
    logger.verbose('project has expired, notifying applicant');
    notifications.set(project.licenceHolder.id, { emailTemplate: 'project-expired', applicant: project.licenceHolder, recipient: project.licenceHolder, expiryDate, ropsDate });
    ...
    const months = action.charAt(action.length - 1);
    logger.verbose(`Project expiring in ${months} months, notifying applicant`);
    notifications.set(project.licenceHolder.id, { emailTemplate: 'project-expiring', applicant: project.licenceHolder, recipient: project.licenceHolder, months, expiryDate, ropsDate });
    ...
    admins
      ...
      .map(profile => {
        if (taskHelper.isExpired(task)) {
          ...
          notifications.set(profile.id, { emailTemplate: 'project-expired', applicant, recipient: profile, expiryDate, ropsDate });

        } else if (taskHelper.isExpiryNotice(task)) {
          const months = action.charAt(action.length - 1);
          logger.verbose(`Project expiring in ${months} months, notifying admin`);
          notifications.set(profile.id, { emailTemplate: 'project-expiring', applicant, recipient: profile, months, expiryDate, ropsDate });
